    ...
    public class SAMAnalyticalSolarControlProfile : GH_SAMVariableOutputParameterComponent
    {
        ...
        protected override GH_SAMParam[] Outputs
        {
            get
            {
                ...
                result.Add(new GH_SAMParam(new global::Grasshopper.Kernel.Parameters.Param_Integer() { Name = "sunHOYs", NickName = "sunHOYs", Description = "Hours the sun was above the horizon (0-based)", Access = GH_ParamAccess.list }, ParamVisibility.Voluntary));
                result.Add(new GH_SAMParam(new global::Grasshopper.Kernel.Parameters.Param_Integer() { Name = "solarDemandHOYs", NickName = "solarDemandHOYs", Description = "Hours the window-plane beam met the solar threshold (0-based)", Access = GH_ParamAccess.list }, ParamVisibility.Voluntary));
                ...
                result.Add(new GH_SAMParam(new global::Grasshopper.Kernel.Parameters.Param_Integer() { Name = "sunHours", NickName = "sunHours", Description = "How many hours the sun was above the horizon", Access = GH_ParamAccess.item }, ParamVisibility.Binding));
                result.Add(new GH_SAMParam(new global::Grasshopper.Kernel.Parameters.Param_Integer() { Name = "solarDemandHours", NickName = "solarDemandHours", Description = "How many hours met the solar threshold", Access = GH_ParamAccess.item }, ParamVisibility.Voluntary));
        ...
        protected override void SolveInstance(IGH_DataAccess dataAccess)
        {
            ...
            {
                AddRuntimeMessage(GH_RuntimeMessageLevel.Warning, string.Format(System.Globalization.CultureInfo.InvariantCulture, "{0} daylight hours carry no outdoor temperature. A value that is not there cannot meet the temperature threshold, so those hours were not claimed by it.", profile.MissingTemperatureHours));
            }

            if (settings.WindConstraintInUse && profile.MissingWindSpeedHours > 0)
            {
                AddRuntimeMessage(GH_RuntimeMessageLevel.Warning, string.Format(System.Globalization.CultureInfo.InvariantCulture, "{0} daylight hours carry no wind speed, so the wind limit could not be checked on them. They are left operable and are NOT reported as high wind — the wind side of this answer rests on an incomplete weather file to that extent.", profile.MissingWindSpeedHours));
            }

            if (profile.ShadeDemandHours == 0)
            {
                AddRuntimeMessage(GH_RuntimeMessageLevel.Remark, "No hour of the year meets this rule at this window. That is a real answer — check the solar threshold against the window's orientation before treating it as a fault.");
            }
            ...
            SetDataList(dataAccess, "sunHOYs", profile.SunHoursOfYear);
            ...
            SetData(dataAccess, "sunHours", profile.SunHours);
